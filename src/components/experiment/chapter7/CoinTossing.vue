...
function createCoins() {
  coins.value = Array.from({ length: coinCount.value })
    .fill(null)
    .map(() => ({ rotation: 0 }));
}
  ...
  <ExperimentBoard :panel-size="70">
    <template #experiment>
      <div class="controls flex justify-center items-center">
        <button :disabled="isFlipping" @click="flipCoins">
          开始抛掷
        </button>
      ...
      <div ref="coinsContainer" class="coins-container">
    ...
    <template #parameter>
      <div class="flex justify-center items-center">
        <label>
          硬币数量:
          <input v-model="coinCount" type="number" min="1" max="50">
        </label>
      </div>
    </template>
    <template #conclusion>
