...
### 二、实验原理
概率质量函数(PMF)是描述离散随机变量取各个可能值的概率的函数。对于离散随机变量X，其PMF $P(X = x)$表示X取值为x的概率。

PMF具有以下性质：
1. 对于所有可能的取值x，$P(X = x) ≥ 0$
2. 所有可能取值的概率之和为1，即$ΣP(X = x) = 1$

### 三、实验操作
左侧控制面板提供了三个滑动条，分别用于调整三个离散点的概率值。每次调整后，中间的PMF图表会实时更新，展示当前概率分布情况。

### 四、观察与思考
- 当调整一个点的概率时，其他点的概率如何变化？
- PMF图表的高度代表什么含义？
- 如何从PMF图表中判断哪个结果发生的可能性最大？
