...
### 二、实验原理
#### 1.概率密度函数(PDF)
概率密度函数描述了连续随机变量在某一点附近的概率密度。对于连续随机变量X，其PDF的$f(x)$满足：
- $f(x) ≥ 0$ 对于所有x
- $\\int_{-\\infty}^{\\infty} f(x) dx = 1$（在整个定义域上的积分等于1）

#### 2.累积分布函数(CDF)
累积分布函数$F(x)$表示随机变量X取值小于等于x的概率，即$F(x) = P(X ≤ x)$。

PDF和CDF的关系：$F(x) = ∫(-∞, x) f(t)dt$，即CDF是PDF的积分。

### 三、实验操作
通过实验界面，你可以观察PDF和CDF曲线的形状，以及它们之间的对应关系。

### 四、观察与思考
- PDF曲线下的面积代表什么？
- 如何从CDF曲线中读取某个区间的概率？
- PDF和CDF各自的特点和应用场景是什么？

### 五、结论
本实验通过可视化展示，帮助我们建立了对连续随机变量PDF和CDF的直观理解，以及它们之间的数学关系。
`;
function onHtmlLoad() {
